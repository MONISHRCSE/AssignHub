...
class _CreateAssignmentScreenState extends State<CreateAssignmentScreen> {
  ...
  Future<void> _pickAndUploadPdf() async {
    ...
    try {
      ...
      final urlData = await _storageSvc.getUploadUrl(fileName: file.name, fileType: 'application/pdf', uploadPurpose: 'ASSIGNMENT');
      final uploadUrl = urlData['upload_url'] as String;
      final fileUrl = urlData['file_url'] as String;

      // step 2: PUT directly to S3
      await _storageSvc.uploadToS3(uploadUrl, file.bytes!, 'application/pdf');
  ...
  Future<void> _pickDeadline() async {
    ...
    if (time == null) return;
    setState(() => _deadline = DateTime(picked.year, picked.month, picked.day, time.hour, time.minute));
  }
  ...
  Future<void> _create() async {
    ...
    try {
      final body = <String, dynamic>{
        ...
        'question_file_url': _uploadedFileUrl,
      };
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            const Text('Submission Type', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF374151))),
            ...
            const Text('Question File (PDF)', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500, color: Color(0xFF374151))),
            const SizedBox(height: 6),
            OutlinedButton.icon(
              onPressed: _fileUploading ? null : _pickAndUploadPdf,
              icon: _fileUploading ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.upload_file_outlined),
              label: Text(_fileUploading ? 'Uploading...' : _fileName ?? 'Attach PDF'),
            ),
            if (_uploadedFileUrl != null)
              Text('✓ Uploaded: $_fileName', style: TextStyle(fontSize: 12, color: Colors.green.shade700)),
            ...
            ListTile(
              ...
              title: Text(_deadline == null ? 'Set Deadline (optional)' : _deadline!.toLocal().toString().substring(0, 16)),
              trailing: _deadline == null ? null : IconButton(icon: const Icon(Icons.clear), onPressed: () => setState(() => _deadline = null)),
              onTap: _pickDeadline,
            ...
            const SizedBox(height: 16),
            SwitchListTile(
              title: const Text('Publish immediately', style: TextStyle(fontSize: 14)),
              subtitle: const Text('Leave off to save as DRAFT', style: TextStyle(fontSize: 12)),
              value: _publishNow,
            ...
            ElevatedButton(
              ...
              child: _loading
                  ...
                  : Text(_publishNow ? 'Create & Publish' : 'Save as Draft', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
            ),
          ],
        ),
      ),
